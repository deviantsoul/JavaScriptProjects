...
function Input_Digit(digit)     {
    ...
    // we are checking to see if Wait_Second_Operandk is true and set Display_Value to the key that was clicked
...
function Handle_Operator(Next_Operator)     {
    ...
    //checks if an operator already exists and if Wait_Second_Operand is true, then update the operator and exits from function
    ...
    keys.addEventListener('click', (event) =>   {
        ...
        const { target } = event;
        // if the element taht was clicked on is not a button, exit the function
